...
def gera_insight(perfil, label, divergence, flag):
    ...
    if flag == "anxiety_risk":
        if perfil == "dissociativo":
            return f"[{momento}] High physiological stress detected, but low subjective report — possible dissociative anxiety pattern."
        else:
            return f"[{momento}] Body showed high stress signals while self-report was low — unrecognized anxiety."
    elif flag == "overreported":
        if perfil == "hiperreativo":
            return f"[{momento}] Subjective distress exceeded physiological response — hyperreactive anxiety pattern."
        else:
            return f"[{momento}] Self-reported stress higher than physiological signals — anticipatory anxiety likely."
    else:
        return f"[{momento}] Perception aligned with physiological state."
...
print(f"  Therapist notification time: {THERAPIST_TIME}")
print(f"{'='*65}")
...
for subject in SUJEITOS:
    ...
    print(f"  {'─'*55}")
    print(f"  Responses today:     {total}")
    print(f"  Avg divergence:      {avg_div:+.3f}")
    print(f"  Dominant flag:       {dom_flag}")
    print(f"  Aligned:             {pct_align}%")
    print(f"  Anxiety risk:        {pct_risk}%")
...
print(f"\n{'='*65}")
print(f"  Saved: explore_nps/subjective_day.csv")
print(f"{'='*65}\n")