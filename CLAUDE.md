...
**IMPORTANTE — `gerar-insight/index.py` NÃO chama o lambda `analisar-texto`.** São duas implementações separadas e independentes da mesma ideia (função `analisar_texto_llm` dentro do próprio `gerar-insight/index.py`, duplicando a lógica de `analisar-texto/index.py`). Isso causou divergência: quando o bug do exemplo numérico e o ZDR foram corrigidos em `analisar-texto` (04/08 e 09/08), a cópia dentro de `gerar-insight` — que é a que roda de verdade na geração de insight real dos pacientes — ficou pra trás, com o bug do exemplo concreto ainda no prompt e sem ZDR nenhum. Achado e corrigido em 2026-08-16 (mesmo fix de placeholder + ZDR replicado na cópia do `gerar-insight`). `analisar-texto` como endpoint HTTP próprio (`POST /analisar-texto`) parece não ser chamado por nada no app — investigar se é código morto antes de confiar na documentação anterior deste arquivo, que descrevia (incorretamente) `gerar-insight` como consumidor desse lambda.
